...
def run_preset_then_2pusle_TDS620B(pg, scope, pulsewidth, delay, high_voltage, preset_pulsewidth = '100ns', preset_voltage = '3000mv',):
	"""
	...
	try: 
		high_voltage = int(high_voltage[:-1])*1000
	except ValueError:
		#means mv is the unit
		high_voltage = int(high_voltage[:-2])
	...
	try: 
		preset_voltage = int(preset_voltage[:-1])*1000
	except ValueError:
		#means mv is the unit
		preset_voltage = int(preset_voltage[:-2])
...
def trial(run_function, run_function_args, base_name, path):
	"""
